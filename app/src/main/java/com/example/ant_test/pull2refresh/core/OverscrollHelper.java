...
public final class OverscrollHelper {
	...
	public static void overScrollBy(final Pull2RefreshBase<?> view, final int deltaX, final int scrollX,
		...
		if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
			final Pull2RefreshMode mode = view.getMode();
			...
			if (mode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0) {
				...
				if (newScrollValue < (0 - fuzzyThreshold)) {
					...
					// then move scroll
					if (mode.showHeaderLoadingLayout() && !view.getStartStyle().isAuto()) {//TODO my modification
						// If we're currently at zero, we're about to start
						// overscrolling, so change the state
						if (currentScrollValue == 0) {
							view.setState(Pull2RefreshState.OVERSCROLLING);
						}
					...
					// Check the mode supports the overscroll direction, and
					// then move scroll
					if (mode.showFooterLoadingLayout() && !view.getEndStyle().isAuto()) {//TODO my modification
						// If we're currently at zero, we're about to start
						// overscrolling, so change the state
						if (currentScrollValue == 0) {
							view.setState(Pull2RefreshState.OVERSCROLLING);
					...
					view.setState(Pull2RefreshState.RESET);
				}
			} else if (isTouchEvent && Pull2RefreshState.OVERSCROLLING == view.getState()) {
